...
class DBConnect:
    ...
    @classmethod
    def get_connection(cls,  host,  user, password,port=3386, database='localhost', pool_name = "my_pool", pool_size = 3 ) -> mysql.connector.pooling.PooledMySQLConnection:
        ...
        if cls._cnxpool is None:
            try:
                ...
                if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
                    ...
                    raise MariaDbConnectError(f"Connessione al DB fallita per user/password : {err}")
                    return None
                elif err.errno == errorcode.ER_BAD_DB_ERROR:
                    my_logger.error("Database does not exist")
                    raise MariaDbConnectError(f"Il Database non esiste: {err}")
                    return None
                else:
                    my_logger.error(err)
                    return None
...
class MariaDbConnectError(Exception):
    pass



if __name__ == "__main__":
    print (f"{pathlib.Path(__file__).resolve().parent}")
